...
public class FormScanBatch {
    ...
    private Slot<ColumnView> addSubRecordCount(FilterLevel filterLevel, ResourceId formId, NodeMatch match) {
        ...
        FormClass subForm = formClassProvider.getFormClass(subFormType.getClassId());
        ...
        SubFormJoin join = new SubFormJoin(primaryKey, parentColumn);
        Slot<ColumnView> dataColumn = addConstantColumn(filterLevel, subForm, new Quantity(1));
    ...
    public Slot<ColumnView> getDataColumn(FilterLevel filterLevel, ResourceId formId, NodeMatch match) {
        if(match.getFieldNode().isSubForm()) {
            ...
            return getDataColumn(filterLevel, formId, match.getFieldComponent());
        }
    }

    public Slot<ColumnView> getDataColumn(FilterLevel filterLevel, ResourceId formId, FieldComponent fieldExpr) {
