...
public class Properties {
    ...
    public static void apply(Element dom, PropMap attrs, boolean isSvgMode) {
        diffProperties(dom, attrs, PropMap.EMPTY, isSvgMode);
    }
     ...
     */
    public static native void diffProperties(Element dom, PropMap attrs, PropMap old, boolean isSvgMode) /*-{
        var name;

        // remove attributes no longer present on the vnode by setting them to undefined
        for (name in old) {
            if (!(attrs && attrs[name]!=null) && old[name]!=null) {
                @Properties::setAccessor(*)(dom, name, old[name], undefined, isSvgMode);
            }
        }
        ...
        for (name in attrs) {
            if (name!=='children' && name!=='innerHTML' && (!(name in old) || attrs[name]!==(name==='value' || name==='checked' ? dom[name] : old[name]))) {
                @Properties::setAccessor(*)(dom, name, old[name], attrs[name], isSvgMode);
            }
        }
    }-*/;
